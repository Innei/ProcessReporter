...
typealias MRMediaRemoteGetNowPlayingInfoFunction = @convention(c) (
	DispatchQueue, @escaping (NSDictionary?) -> Void) -> Void
...
enum Command {
	case get, getRaw, mediaCommand, seek
}

@objc public class NowPlaying: NSObject {
	@objc public static func getNowPlayingInfo() -> NSDictionary? {
		...
		autoreleasepool {
			...
			let getMRMediaRemoteGetNowPlayingApplicationPID = unsafeBitCast(
				CFBundleGetFunctionPointerForName(bundle, "MRMediaRemoteGetNowPlayingApplicationPID" as CFString),
			...
			{ information in
				...
				let startTime = (info["kMRMediaRemoteNowPlayingInfoStartTime"] as? NSNumber)?.doubleValue ?? 0
				...
				let radioStationIdentifier = info["kMRMediaRemoteNowPlayingInfoRadioStationIdentifier"] as? String
				let radioStationHash = info["kMRMediaRemoteNowPlayingInfoRadioStationHash"] as? String
					
				// Extract supported features
				let supportsFastForward15Seconds = info["kMRMediaRemoteNowPlayingInfoSupportsFastForward15Seconds"] as? NSNumber
				let supportsRewind15Seconds = info["kMRMediaRemoteNowPlayingInfoSupportsRewind15Seconds"] as? NSNumber
